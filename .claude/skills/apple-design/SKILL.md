...
## How THIS repo applies it (read first)

Vendored from [emilkowalski/skills](https://github.com/emilkowalski/skills)
(`skills/apple-design`), unmodified below this section. This preamble is ours:
it says which sections bind here, because a few of them argue against decisions
this codebase has already made deliberately.

**Binding — a reviewer may raise these as findings:**

| § | Rule | Where it lives here |
|---|---|---|
| 1 | Feedback on pointer-down, not release | global `:active` in `theme.css` |
| 3 | Never lock out input during a transition | — |
| 7 | Enter and exit along the same path; anchor to the source | `.scrim` / `.drawer-panel` / `.modal-panel` |
| 14 | Reduced motion / transparency / contrast | the three `@media` blocks in `theme.css` |
| 15 | Tracking and leading are size-specific | `DESIGN.md` → Type |
| 16 | The eight principles — as vocabulary for design argument | `DESIGN.md` |
| 17 | Prototype interactively; verify in a real browser | the cycle's headful-verification rule |

**Not binding here — do NOT open findings against these:**

- **§12 (translucent materials, `backdrop-filter`, vibrancy).** Directly
  contradicts `docs/dev/DESIGN.md` → "No decorative styling": this app retired
  gradients and glows on purpose, and `--gradient-*` / `--glow-accent` resolve
  to solid values. Do not reintroduce a material layer. Its *hierarchy* points
  (dim to focus, one translucent surface at a time) already hold via `--overlay`.
- **§§2, 5, 6, 9, 10, 11 (1:1 drag, velocity handoff, momentum projection,
  rubber-banding, gesture disambiguation, frame-level smoothness).** There are
  no custom gesture surfaces in this codebase and no spring library. The one
  gesture surface — React Flow in `components/Relationships/` — is the
  library's concern. Adding a spring dependency to a static-export admin tool
  to satisfy these is unearned; if a genuine drag surface ever ships, they
  become binding for it.
- **§4's spring values / §13 (haptics, sound).** No audio or Vibration API in
  an admin control plane.

The short version: this skill is loaded here for **response, interruptibility,
spatial consistency, accessibility, typography, and the design vocabulary** —
not for Apple's visual material language, which `DESIGN.md` overrides.
