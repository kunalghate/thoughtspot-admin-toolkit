...
def _group_row_to_dict(g: CachedGroup, member_count: int) -> dict:
    return {
        "ts_guid": g.ts_guid,
        "name": g.name,
...
def list_groups(
    *,
    ...
    with Session(_db.get_engine()) as session:
        ...
        base = (
            select(CachedGroup, member_count_col.label("member_count"))
            .outerjoin(
                member_counts,
...
def get_group_detail(*, cluster_id: str, ts_guid: str, org_id: int | None = None) -> dict | None:
    ...
    with Session(_db.get_engine()) as session:
        ...
        out = _group_row_to_dict(group, member_count)
        out["members"] = [
            {
                "ts_guid": u.ts_guid,
                "username": u.username,
