...
def validate_cluster_url(url: str) -> str:
    ...
    if parsed.scheme != "https":
        raise ConfigInvalidError(f"ThoughtSpot URL must use HTTPS, got: {parsed.scheme!r}")

    hostname = parsed.hostname or ""

    # Block loopback
    if hostname in ("localhost", "127.0.0.1", "::1"):
        raise ConfigInvalidError("ThoughtSpot URL must not point to localhost.")

    # Block private IP ranges (SSRF prevention)
    try:
        addr = ipaddress.ip_address(hostname)
        if addr.is_private or addr.is_loopback or addr.is_link_local:
            raise ConfigInvalidError(f"ThoughtSpot URL must not point to a private/internal IP address: {hostname}")
    except ValueError:
