...
async def get_sync_status(org_id: int = 0) -> list[EntitySyncStatus]:
    ...
    with get_session() as session:
        rows = session.exec(
            select(SyncLog)
            .where(SyncLog.cluster_id == cluster_id, SyncLog.org_id == org_id)
        ).all()
    ...
    for entity in VALID_ENTITIES:
        ...
        result.append(EntitySyncStatus(
            entity_type=entity,
...
async def trigger_sync(
    ...
    job_id = create_job(job_type=f"sync:{entity_type}", parameters={
...
async def trigger_sync_all(
    ...
    for entity in standard_entities:
        job_id = create_job(job_type=f"sync:{entity}", parameters={
            "entity_type": entity,
            "org_id": org_id,
        })
